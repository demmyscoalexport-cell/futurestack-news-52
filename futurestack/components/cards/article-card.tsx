...
export function ArticleCard({
  ...
  const authorName = article.author?.name || "FutureStack AI";
  ...
  if (variant === "horizontal") {
    return (
      <Card
        className={cn(
          "group flex overflow-hidden transition-all hover:shadow-md hover:border-primary/30",
          className,
        )}
      >
        <Link href={`/news/${article.slug}`} className="flex flex-1">
          ...
          <div className="flex flex-1 flex-col justify-center p-4">
            <Badge
              variant="outline"
              className={cn("text-xs w-fit", categoryColors[article.category])}
            >
              {categoryLabels[article.category]}
            </Badge>

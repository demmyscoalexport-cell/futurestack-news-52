...
import {
  Users, MessageCircle, Star, Globe, Heart, ArrowRight,
} from "lucide-react";
...
const DISCUSSIONS = [
  {
    ...
    timeAgo: "2h ago",
    hot: true,
    href: "/news/best-ai-tools-slow-internet-nigeria",
  },
  {
    id: 2,
...
const COMMUNITIES = [
  { name: "Nigerian Tech Builders", members: "4.2K", icon: "🇳🇬", href: "/discover?section=nigeria" },
  { name: "Kenyan Founders", members: "2.8K", icon: "🇰🇪", href: "/discover?section=kenya" },
  { name: "Ghana Creators", members: "1.9K", icon: "🇬🇭", href: "/discover?section=ghana" },
  { name: "African AI Enthusiasts", members: "6.1K", icon: "🤖", href: "/discover?section=trending" },
  { name: "WhatsApp Commerce Guild", members: "3.4K", icon: "💬", href: "/workflows" },
  { name: "African Freelancers", members: "5.7K", icon: "💼", href: "/opportunities" },
  { name: "Student Tech Africa", members: "8.2K", icon: "🎓", href: "/deals" },
  { name: "Africa SaaS Founders", members: "2.1K", icon: "🚀", href: "/stack-builder" },
];
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Button variant="outline" size="sm" className="h-8 text-xs" asChild>
                    <Link href="/signup">Start Discussion</Link>
                  </Button>
                </div>
                <div className="space-y-3">
                  {DISCUSSIONS.map((d) => (
                    <Link
                      key={d.id}
                      href={d.href}
                      className="block rounded-xl border border-border/50 bg-card p-4 hover:border-primary/30 transition-all group"
                    >
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {CONTRIBUTORS.map((c) => (
                    <Link
                      key={c.name}
                      href="/signup"
                      className="flex items-center gap-3 rounded-xl border border-border/50 bg-card p-3.5 hover:border-primary/30 transition-all group"
                    >
                ...
                <div className="space-y-1">
                  {COMMUNITIES.map((c) => (
                    <Link
                      key={c.name}
                      href={c.href}
                      className="flex items-center gap-2.5 rounded-lg p-2 hover:bg-secondary/40 transition-colors group"
                    >
                      <span className="text-base">{c.icon}</span>
                      <div className="min-w-0 flex-1">
                        <p className="text-xs font-medium text-foreground group-hover:text-primary transition-colors truncate">{c.name}</p>

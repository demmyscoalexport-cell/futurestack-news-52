...
export default function AffiliatesClient() {
  ...
  const [dailyTrend, setDailyTrend] = useState<DailyTick[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    if (statsRes.ok) {
      ...
      setDailyTrend(stats.dailyTrend || []);
    }
    setLoading(false);
  }, []);
  ...
  function startNew() {
    setEditing({ id: null, tool_id: "", tool_name: "", affiliate_url: "", partner_name: "", commission_rate: "0", notes: "" });
  }
  ...
  async function handleDelete(affiliateId: string, toolName: string) {
    ...
    if (res.ok) { setSuccessMsg("Deleted"); setTimeout(() => setSuccessMsg(null), 3000); load(); }
  }
  ...
  const maxBarClicks = Math.max(...dailyTrend.map(d => d.clicks), 1);
      ...
      <div className="p-6 space-y-6">
        ...
        {dailyTrend.length > 0 && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4">Daily Clicks — Last 30 Days</h3>
            <div className="flex items-end gap-1 h-24">
              {dailyTrend.map((d) => {
                const heightPct = Math.max((d.clicks / maxBarClicks) * 100, 4);
                ...
                return (
                  <div key={d.day} className="flex-1 flex flex-col items-center gap-1 group relative">
                    ...
                    <div className="absolute bottom-full mb-1 hidden group-hover:flex flex-col items-center pointer-events-none z-10">
                      <div className="bg-slate-800 border border-slate-700 rounded-lg px-2 py-1 text-xs text-white whitespace-nowrap shadow-lg">
                        {dateLabel}: <span className="font-bold text-violet-400">{d.clicks}</span> clicks
                      </div>
                    </div>
                  </div>
                );
            ...
            <div className="flex justify-between text-xs text-slate-600 mt-2">
              <span>{new Date(dailyTrend[0]?.day).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
              <span>{new Date(dailyTrend[dailyTrend.length - 1]?.day).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setEditing(null)}>
            <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-4">
                {[
                  { label: "Affiliate URL *", field: "affiliate_url" as const, placeholder: "https://example.com/?ref=futurestack" },
                  { label: "Partner Name *", field: "partner_name" as const, placeholder: "e.g. Notion" },
                ...
                filtered.map(t => {
                  ...
                  <tr key={t.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      {t.affiliate_url ? (
                        t.is_active
                          ? <span className="inline-flex items-center gap-1 text-xs font-semibold bg-emerald-900/40 text-emerald-400 border border-emerald-700/40 px-2 py-0.5 rounded-full"><Check className="h-2.5 w-2.5" /> Active</span>
                          : <span className="inline-flex items-center gap-1 text-xs font-semibold bg-slate-800 text-slate-500 border border-slate-700 px-2 py-0.5 rounded-full"><X className="h-2.5 w-2.5" /> Paused</span>
                      ) : (
                        <span className="text-xs text-slate-600 italic">No link</span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        {t.affiliate_url && (
                          <a href={`/api/affiliate/${t.slug}`} target="_blank" rel="noopener noreferrer" title="Test redirect"
                            className="p-1.5 text-slate-500 hover:text-blue-400 rounded-lg hover:bg-slate-800 transition-colors">
                            <ExternalLink className="h-3.5 w-3.5" />
                          </a>
                        )}
                        {t.affiliate_id && (
                          <button onClick={() => toggleActive(t.affiliate_id!, t.is_active!)} title={t.is_active ? "Pause" : "Activate"}
                        ...
                        {t.affiliate_id && (
                          <button onClick={() => handleDelete(t.affiliate_id!, t.name)} title="Delete"
